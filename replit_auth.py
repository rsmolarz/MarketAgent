...
from flask import g, session, redirect, request, render_template, url_for, flash, current_app
...
def is_user_whitelisted(email):
    """Check if email is in the whitelist"""
    from models import Whitelist
    if not email:
        return False
    return Whitelist.query.filter_by(email=email.lower()).first() is not None
...
def make_replit_blueprint():
    ...
    @replit_bp.route("/not-whitelisted")
    def not_whitelisted():
        return render_template("not_whitelisted.html"), 403
...
def save_user(user_claims):
    ...
    if admin_emails and user.email:
